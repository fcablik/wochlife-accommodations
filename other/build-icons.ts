...
async function generateIconFiles() {
	...
	const typeOutputFilepath = path.join(outputDir, 'nameTypes.d.ts')
	const nameStringsOutputFilepath = path.join(outputDir, 'nameStrings.ts')
	...
	const currentTypes = await fsExtra
		...
		.catch(() => '')
	const currentNameStrings = await fsExtra
		.readFile(nameStringsOutputFilepath, 'utf8')
		.catch(() => '')
	...
	const nameStringsUpToDate = iconNames.every(name =>
		currentNameStrings.includes(`"${name}"`),
	)

	if (spriteUpToDate && typesUpToDate && nameStringsUpToDate) {
	...
	const nameStringsOutputContent = `// This file is generated by npm run build:icons
	...
	`
	const nameStringsChanged = await writeIfChanged(
		nameStringsOutputFilepath,
		nameStringsOutputContent,
	)
	logVerbose(
		`Manifest saved to ${path.relative(cwd, nameStringsOutputFilepath)}`,
	)
...
`,
	...
	if (spriteChanged || typesChanged || nameStringsChanged || readmeChanged) {
