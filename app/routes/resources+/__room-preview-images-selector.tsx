...
export async function action({ request, params }: DataFunctionArgs) {
	...
	const updatedRoom = await prisma.room.update({
		...
		data: {
			roomPreviewImages: {
				set: [],
				connect: roomPreviewImages.length
					? roomPreviewImages[0].map(gallerImageId => ({ id: gallerImageId }))
					: [],
			},
		},
	})
...
export function PreviewImagesSelector({
	...
	room?: SerializeFrom<
		Pick<Room, 'id'> & {
			roomPreviewImages: Array<Pick<RoomsGalleryImage, 'id'>>
		}
	>
	...
	const [selectedGalleryImages, setSelectedGalleryImages] = useState<string[]>(
		room?.roomPreviewImages?.map(gallery => gallery.id) ?? [],
	)
	...
	const [form, fields] = useForm({
		...
		// defaultValue: {
		// 	roomPreviewImages: room?.roomPreviewImages
		// 		? (room?.roomPreviewImages).map(galleryImage => galleryImage.id)
					...
					{allImages.length ? (
						<>
							<Field
								...
								inputProps={{
									...conform.input(fields.roomPreviewImages, {
										ariaAttributes: true,
									}),
									type: 'hidden',
